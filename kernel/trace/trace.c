				     ...
				     unsigned int trace_ctx)
{
	...
	if (tr->trace_flags & TRACE_ITER(PERF_CACHE)) {
		*value++ = PERF_VALUE_CACHE(do_perf_cache_misses());
		entries--;
	}

	if (entries && tr->trace_flags & TRACE_ITER(PERF_CYCLES)) {
		*value++ = PERF_VALUE_CYCLES(do_perf_cpu_cycles());
		entries--;
	}
